...
6. 引入 Task Blackboard，作为单次任务内多 Agent 协作的结构化共享工作台。
7. 支持多层 Memory：全局 Session / Working / Short-term / Long-term Memory，以及子 Agent 独立 Memory。
8. 提供 Context Manager，统一管理上下文选择、压缩、预算、证据和权限。
9. 提供 Evaluation Layer，对输出真实性、正确性、完整性、一致性和风险进行评估。
10. 对写入类、高风险类、低置信度类操作提供 Human-in-the-loop 机制。
11. 当前暂不要求多租户，但架构需要预留租户、部门、权限域扩展空间。
...
2. plan：候选 Task Plan、已冻结 Task Plan、版本和预算。
3. work items：待执行、执行中、已完成、失败、暂停等待确认的子任务。
...
| Workflow Engine | 读取 work item，写入节点状态和 artifact |
...
1. 子 Agent 启动时只获得 assigned work item 和 Context Manager 生成的局部 Blackboard view。
2. 子 Agent 完成后返回结构化结果，由 Workflow Engine 写入 Blackboard。
3. Context Manager 在每个节点前读取 Blackboard 当前状态、Memory 候选和工具证据，生成 Context Bundle。
4. Evaluation Layer 基于 Blackboard 中的 claim、evidence 和 trace 判断是否通过。
5. 任务结束后，Blackboard 被关闭并归档；只有经过 Memory Candidate Pipeline 的摘要或经验才可进入长期 Memory。
...
在 Blackboard 架构下，Workflow Engine 不只是顺序调用节点，还负责把节点状态、节点输入摘要、节点输出、失败原因和等待项写入 Task Blackboard。Workflow Engine 应把 Blackboard 作为任务运行态的唯一事实源，避免节点之间通过隐式变量或 Prompt 文本传递关键状态。
...
### 4.6 Tool Registry 与 Tool Gateway
...
flowchart TD
    ...
    N --> O["Write Work Items to Blackboard"]
    O --> P["Deterministic Dispatch Sub-tasks"]
    P --> Q["Sub-agents Write Artifacts and Claims"]
    Q --> R["Validate Result Schema"]
    R --> S["Evaluate Claims, Artifacts and Conflicts"]
    S --> T["Write Evaluation Results to Blackboard"]
    T --> U["Build Aggregation Context from Blackboard"]
    U --> V["Check Consistency and Confidence"]
    V --> W{"Need Review or Retry?"}
    W -- Yes --> X["Retry / Alternative Agent / Human Gate"]
    X --> O
    W -- No --> Y["Aggregate Final Report Candidate"]
    Y --> Z["Evaluate Final Report"]
    Z --> AA["Close Blackboard and Archive Trace"]
    AA --> AB["Create Memory Candidates if Allowed"]
    AB --> AC["Return Final Answer"]
```
...
在 Blackboard 架构下，主 Agent 不直接“收集一堆自然语言结果”再汇总，而是从 Task Blackboard 中读取已验证的 artifact、claim、evidence、conflict 和 evaluation result。子 Agent 返回给主 Agent 的也不应是完整上下文，而应是结构化 result ref，例如 `blackboard://task_001/artifacts/risk_result_v2`。
...
2. Workflow Engine 从 Task Blackboard 读取 assigned work item 和当前节点所需 artifact ref。
3. 子 Agent 根据 Agent Profile 读取自己的 Local Session Memory 和 Local Working Memory。
4. 根据当前 Workflow 节点读取必要的 Local Short-term / Long-term Memory。
5. 若需要全局规则或跨 Agent 信息，通过 Memory Manager 读取 Global Memory。
6. Memory Manager 根据权限、场景、任务目的和数据分级返回候选记忆，Context Manager 再结合 Blackboard view 决定是否进入最终 Context Bundle。
...
```json
{
  ...
  "goal": {
    "user_goal": "Review this contract and provide risk recommendations",
    "confirmed_constraints": [
      "Focus on payment and privacy clauses."
    ]
  },
  ...
  "work_items": [
    {
      ...
      "depends_on": ["sub_001"],
      "input_refs": [
        "blackboard://task_001/artifacts/parsed_document_v1"
      ]
    }
  ],
  "artifacts": [
    {
      "artifact_id": "risk_result_v1",
      "type": "risk_assessment",
      "owner_agent_id": "contract_risk_agent",
      "status": "candidate",
      "source_refs": [
        "doc_001#page=5#clause=4.2"
      ]
    }
  ],
  "claims": [
    {
      "claim_id": "claim_payment_period_90_days",
      "status": "supported",
      "evidence_refs": [
        "doc_001#page=5#clause=4.2"
      ],
...
```json
{
  ...
  "node_id": "risk_assessment",
  "status": "candidate",
  "content_ref": "object://blackboard/task_001/artifacts/risk_result_v1.json",
  "source_refs": [
    "blackboard://task_001/artifacts/parsed_document_v1",
    "policy:finance_policy_v3#payment_terms"
  ],
  "visibility_scope": [
    "main_agent",
    "contract_risk_agent",
...
### 15.7 Agent Local Memory Record
...
```json
{
  ...
  "task_id": "task_001",
  "blackboard_refs": [
    "blackboard://task_001/artifacts/risk_result_v1",
...
5. 实现 Task Blackboard 基础版：work item、artifact、claim、evaluation result、event log。
6. 使用代码 Workflow 或已锁定的配置 Workflow。
7. 实现 Sub Agent 基类、固定 Agent Registry 和基础 Role Profile。
8. 实现轻量 Workflow Engine：顺序节点、简单分支、节点状态、失败返回，并写入 Blackboard。
9. 实现 Context Manager 基础版：从 Blackboard view 构造 Context Bundle，支持预算控制、来源标注和 evidence_ref。
10. 实现 Evaluation 基础版：Schema、规则、必填项、证据引用检查，并写回 Blackboard。
11. 实现 Skill Registry 基础版：固定 Skill、固定版本、单元测试。
12. 实现 Tool Gateway 基础版：只读工具和少量写入工具预览，结果摘要写入 Blackboard。
13. 写入类工具必须经过 Human Gate，支持 idempotency_key。
14. 只启用 Session Memory、Working Memory 摘要和 Task Blackboard。
15. 暂不启用 Agent 自动写 Long-term Memory。
16. 记录 Audit Log、Workflow Trace、Blackboard Event 和关键 Context Snapshot。
17. 建立 10 到 30 个 Golden Cases，覆盖成功、缺失信息、低证据、高风险和失败路径。
...
7. Factuality / Correctness / Consistency / Safety 评估器。
8. Workflow 版本管理、签名、dry run 和回滚。
...
3. 引入 Task Blackboard 作为单次任务内多 Agent 协作的结构化共享工作台，所有 work item、artifact、claim、evidence、conflict、decision 和 evaluation result 都应进入 Blackboard。
4. Task Blackboard 不是 Long-term Memory。任务结束后关闭并归档，只有通过 Memory Candidate Pipeline 的摘要或经验才可进入长期记忆。
5. 子 Agent 不直接互相调用，也不读取完整 Blackboard；它们通过 assigned work item 和 Context Manager 生成的局部 Blackboard view 工作。
6. 子 Agent 数量由 Scenario Pack / Agent Registry 自定义，不在代码中写死。
7. 子 Agent 角色由 Role Profile 自定义，不在 Prompt 或代码里硬编码。
8. 主 Agent 保留有限自主规划能力，但只能在注册能力范围内规划，并且所有计划必须经过 Plan Validator 后写入冻结 Blackboard plan。
9. 子 Agent 默认绑定固定 Workflow，关键流程优先使用代码 Workflow 或已签名、已审批的配置 Workflow。
10. Workflow 采用混合定义：拓扑配置化，节点实现代码化；分支条件使用受限 DSL，禁止任意代码执行。
11. Tool Registry 和 Tool Gateway 作为安全边界，所有工具调用必须通过 Tool Gateway，工具结果摘要先写入 Blackboard，再由 Context Manager 注入上下文。
12. 写入类工具必须经过 prepare / preview / approve / commit 流程，并支持幂等、审计和补偿策略。
13. Context Manager 作为上下文编译层，统一处理 Blackboard view、Memory 候选、压缩、预算、权限、证据和快照。
14. Context Snapshot 必须脱敏、加密、分级访问，并设置原始数据保留期。
15. Evaluation Layer 采用风险分级策略，评估结果、冲突和 gate_action 写回 Blackboard。
16. `overall_score` 只能辅助观察，安全、权限、Schema、Human Gate 和关键证据缺失应作为硬闸门。
17. 借鉴 Hermes 的小容量常驻记忆、Session Search 和 Skills as procedural memory，但采用企业级结构化、权限化和可审计实现。
18. Memory 分层设计仍保留，但 Phase 1 只启用 Session / Working Memory 摘要和 Task Blackboard，暂不允许 Agent 自动写 Long-term Memory。
19. Long-term Memory 写入必须通过 Memory Candidate Pipeline、Evaluation Gate、Memory Policy 和必要审批。
20. Core Memory Snapshot 只保存高价值、低风险、稳定、常用的信息摘要，并通过 Context Manager 按预算注入。
21. 通过 Schema、Validator、Task Blackboard、Context Snapshot、Evaluation Result、Golden Case、Audit Log 和 Workflow Trace 保证稳定、精准和可回放。
22. 平台化能力应在单场景稳定后再扩展，多场景配置、长期记忆、Episode Search、可视化治理和跨场景经验迁移都应后置。
